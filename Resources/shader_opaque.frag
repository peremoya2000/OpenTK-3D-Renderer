...
vec3 shadeFragment(vec4 albedo, vec3 lightDir, vec3 lightColor)
{
    vec3 ambient = lightColor * material.ambientTint * albedo.rgb;

    float diffIntensity = max(dot(-normal, lightDir), 0.0);
    vec3 diffuse = lightColor * (diffIntensity * albedo.rgb * material.diffuseTint);
...
vec3 directionalLight(Light light, vec4 albedo)
{
...
vec3 pointLight(Light light, vec4 albedo)
{
...
void main()
{
    vec4 albedo = texture(material.mainTex, texCoord);
    if (albedo.a < 0.5)
    {
        discard;        
    }
